...
- **Пагинация статей** (по 6 статей на странице)
...
Блог использует систему с JSON метаданными. Чтобы добавить новую статью:

### Шаг 1: Создайте HTML файл статьи

Создайте новый файл в `public/posts/` (например, `my-new-post.html`). Используйте шаблон ниже.

### Шаг 2: Добавьте метаданные в JSON

Откройте `public/data/articles.json` и добавьте новую запись в массив `articles`:
...
**Важно:** Новые статьи добавляются в начало массива (сверху), чтобы отображались первыми.

### Шаг 3: Добавьте изображения (опционально)

Если нужны изображения:
- Поместите их в `public/assets/img/`
- В HTML используйте: `<img src="../assets/img/your-image.png" alt="Описание">`
...
Статья автоматически появится на главной странице!

## Функционал блога

### Пагинация

- Главная страница отображает последние статьи с пагинацией
- По умолчанию 6 статей на странице
- Навигация по страницам с URL параметром `?page=2`
- Изменить количество можно в `public/assets/js/articles.js` (параметр `articlesPerPage`)
